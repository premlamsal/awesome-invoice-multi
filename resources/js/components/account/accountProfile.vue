                            ...
                            <div class="row">
                                <div class="col-md-3 ml-auto">
                                    <h5 class="text-warning">
                                        Rs. {{account.opening_balance}}<br /><small
                                            >Opening Bal</small
                                        >
                                    </h5>
                                </div>
                                ...
                                >
                                    <h5 class="text-success">
                                        Rs. {{account.invoice_amount}}<br /><small>Total Sales</small>
                                    </h5>
                                </div>
                                 <div
                                    class="col-md-3 ml-auto mr-auto"
                                >
                                    <h5 class="text-success">
                                        Rs. {{account.paid_amount}}<br /><small>Paid</small>
                                    </h5>
                                </div>
                                <div class="col-md-3 mr-auto">
                                    <h5 class="text-danger">
                                        Rs. {{account.balance_due}}<br /><small
                                            >Due Balance</small
                                        >
                                    </h5>
                                </div>
...
export default {
    ...
    methods: {
        ...
        getCustomerPayments() {
            axios
                .get("/api/account/payments/" + this.account_id)
                .then(response => {
        ...
        addPayment() {
            ...
            axios
                .post("/api/account/add-payment", formData, config)
        ...
        editViewPayment(payment_id) {
            ...
            this.errors = ""; //clearing errors
            axios
                .get("/api/account/payment/" + payment_id)
                .then(response => {
        ...
        updatePayment() {
            ...
            axios
                .post("/api/account/update-payment", formData, config)
                .then(function(response) {
                    currObj.output = response.data.msg;
                    currObj.status = response.data.status;
                    // alert(currObj.status);
        ...
        deletePayment(id) {
            ...
            this.$swal({
                ...
                if (result.value) {
                    axios
                        .delete("/api/account/delete-payment/" + id)
                        .then(function(response) {
                            currObj.output = response.data.msg;
                            currObj.status = response.data.status;
                            // alert(currObj.status);
