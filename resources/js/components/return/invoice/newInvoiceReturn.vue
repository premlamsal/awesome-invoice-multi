  ...
  <div>
    ...
    <div class="card shadow mb-4">
      <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Create Return Invoice</h6>
      </div>
      <div class="card-body">
        <div class="row">
          <div class="col-sm-4">
            ...
            <div class="form-group" style="position: relative;">
              ...
              <div class="customer-search-suggestion" v-if="queryResults!= null ">
          ...
          <div class="col-sm-4">
            <div class="form-group">
              <label>Note</label>
              <textarea v-model="info.note" v-bind:class="[errors['info.note'] ?  'is-invalid' : '' , 'form-control']"></textarea>
              <span v-if="errors['info.note']" :class="['errorText']">{{errors['info.note'][0]}}</span>
              <!-- <div v-if="errors['info.note']" class="invalid-tooltip">
                {{errors['info.note'][0]}}
              </div> -->
            ...
            <div class="row">
              <div class="col-sm-6">
                ...
                <date-picker v-model="info.return_invoice_date" :config="options" v-bind:class="[errors['info.return_invoice_date'] ?  'is-invalid' : '' , 'form-control']"></date-picker>
                <span v-if="errors['info.return_invoice_date']" :class="['errorText']">{{errors['info.return_invoice_date'][0]}}</span>
              </div>
              <div class="col-sm-6">
                <label>Due Date</label>
                <date-picker v-model="info.due_date" :config="options" v-bind:class="[errors['info.due_date'] ?  'is-invalid' : '' , 'form-control']"></date-picker>
                <span v-if="errors['info.due_date']" :class="['errorText']">{{errors['info.due_date'][0]}}</span>
              </div>
            </div>
          </div>
        </div>
        <div class="return_invoice">
          ...
          <!-- end of return_invoice head-->
          <div class="return_invoice-body">
            <div class="return_invoice-items" v-for="(item,index) in items" v-bind:key="item.id">
              <div class="row">
                ...
                <div class="col-md-3">
                  <div class="auto-complete-product-container">
                    <div class="form-group">
                      ...
                      <!--  suggestion block -->
                      <div class="product-search-suggestion-return_invoice" >
                        <ul>
            ...
            <!-- end of return_invoice items-->
          </div>
          <!-- end of return_invoice body-->
          <div class="return_invoice-foot">
            <div class="row">
              <div class="col-md-2">
          ...
          <!-- end of return_invoice foot -->
        </div>
        <!-- end of return_invoice -->
        <button class="btn btn-success" @click="createReturnInvoice">Create</button>
        <router-link to="/return_invoices" class="btn btn-danger">Close</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetchStore() {
      ...
      .then(function(response) {
        ...
        Vue.set(
          currObj.store,
          "return_invoice_id_count",
          response.data.store.return_invoice_id_count
        );

        currObj.return_invoice_number = currObj.store.return_invoice_id_count.split("-");

        currObj.return_invoice_number[1] = parseInt(currObj.return_invoice_number[1]) + 1;

        currObj.return_invoice_number = currObj.return_invoice_number.join("-");
    ...
    calLineTotal(index) {
      // alert(this.return_invoices[index].price);
    ...
    createReturnInvoice() {
      ...
      let currObj = this;
      axios
        .post("/api/return_invoice", { info: this.info, items: this.items })
        .then(function(response) {
    ...
    //type:'error','success','warn'//title:'title of message'//message:'body of message'
  ...
  watch: {
    ...
    "info.return_invoice_date": function(val, oldVal) {
      // console.log('estimate_date changes');
      if (this.errors != "") {
        this.errors["info.return_invoice_date"] = false;
        // this.errors["info.return_invoice_date"][0] = null;
      }
    },
...
.product-search-suggestion-return_invoice {
  position: absolute;
  /* background: #f4f3ef; */
  width: 100%;
  color: #212120;
...
.product-search-suggestion-return_invoice ul {
  list-style: none;
  margin: 0px;
  padding: 0px;
}

.product-search-suggestion-return_invoice ul li {
  padding: 0.7em;
  cursor: pointer;
  background: #f4f3ef;
}

.product-search-suggestion-return_invoice ul li:hover {
  background: #51cbce;
  color: white;
}

.product-search-suggestion-return_invoice::-webkit-scrollbar-track {
  -webkit-box-shadow: inset 0 0 6px rgba(0, 0, 0, 0.3);
  background-color: #f5f5f5;
}

.product-search-suggestion-return_invoice::-webkit-scrollbar {
  width: 6px;
  background-color: #f5f5f5;
}

.product-search-suggestion-return_invoice::-webkit-scrollbar-thumb {
  background-color: #000000;
}
</style>
