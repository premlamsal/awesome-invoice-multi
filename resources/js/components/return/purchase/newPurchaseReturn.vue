  ...
  <div>
    ...
    <div class="card shadow mb-4">
      ...
      <div class="card-body">
        <div class="row">
          <div class="col-sm-4">
            <div class="form-group">
              <label>Return Purchase No. (auto generated)</label>
              {{return_purchase_number}}
            </div>
            <div class="form-group" v-if="info.supplier_id!=null">
              <label>Actual Return Purchase Number(Reference Return Purchase ID)</label>
              <span>{{info.supplier_short_name}} - {{info.return_purchase_reference_number}}</span>
              <input type="text" v-model="info.return_purchase_reference_number" class="form-control"/>
              <span v-if="errors['info.return_purchase_reference_number']" :class="['errorText']">
                {{errors['info.return_purchase_reference_number'][0]}}
                <br />
              </span>
              </div>
        ...
        <!-- end of purchase -->
        <button class="btn btn-success" @click="createReturnPurchase">Create</button>
  ...
  methods: {
     fetchStore() {
      ...
      .then(function(response) {
        ...
        Vue.set(
          currObj.store,
          "return_purchase_id_count",
          response.data.store.return_purchase_id_count
        );

        currObj.return_purchase_number = currObj.store.return_purchase_id_count.split("-");

        currObj.return_purchase_number[1] = parseInt(currObj.return_purchase_number[1]) + 1;

        currObj.return_purchase_number = currObj.return_purchase_number.join("-");
    ...
    createReturnPurchase() {
      ...
      let currObj = this;
      axios
        .post("/api/return_purchase", { info: this.info, items: this.items })
        .then(function(response) {
    ...
    autoComplete: _.debounce(function() {
      ...
      if (this.info.supplier_name === "") {
        ...
        this.info.return_purchase_reference_number='';
      } else {
        axios
