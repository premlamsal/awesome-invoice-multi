  ...
  <div>
    ...
    <div class="card shadow mb-4">
      <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Edit Return Puchase</h6>
      </div>
      <div class="card-body">
        <div class="row">
          <div class="col-sm-4">
            <label>ReturnPuchase No.</label>
          ...
          <div class="col-sm-4">
            ...
            <div class="row">
              <div class="col-sm-6">
                <label>ReturnPuchase Date</label>
                <date-picker
                  v-model="info.return_purchase_date"
                  :config="options"
                  :class="['form-control']"
                ></date-picker>
                <!-- <input type="date" v-model="info.return_purchase_date" class="form-control"> -->
                <span
                  v-if="errors['info.return_purchase_date']"
                  :class="['errorText']"
                  >{{ errors["info.return_purchase_date"][0] }}</span
                >
              </div>
              <div class="col-sm-6">
        ...
        <div class="return_purchase">
          ...
          <!-- end of return_purchase head-->
          <div class="return_purchase-body">
            <div
              class="return_purchase-items"
                ...
                <div class="col-md-3">
                  <div class="auto-complete-product-container">
                    <div class="form-group">
                      ...
                      <!--  suggestion block -->
                      <div class="product-search-suggestion-return_purchase">
                        <ul>
                          <li
            ...
            <!-- end of return_purchase items-->
          </div>
          <!-- end of return_purchase body-->
          <div class="return_purchase-foot">
            <div class="row">
              <div class="col-md-2">
          ...
          <!-- end of return_purchase foot -->
        </div>
        <!-- end of return_purchase -->
        <button class="btn btn-success" @click="updateReturnPuchase">Update</button>
        <router-link to="/return_purchases" class="btn btn-danger">Close</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    calLineTotal(index) {
      // alert(this.return_purchases[index].price);
    ...
    updateReturnPuchase() {
      ...
      axios
        ...
        .then(function (response) {
          ...
          currObj.$router.push({ name: "returnPurchases" });
    ...
    fetchReturnPuchase() {
      ...
      axios
        .get("/api/return_purchase/" + this.id)
        .then(function (response) {
          Vue.set(currObj.info, "return_purchase_no", response.data.return_purchase.id),
            Vue.set(currObj.info, "note", response.data.return_purchase.note),
            Vue.set(
              currObj.info,
              "custom_return_purchase_id",
            ...
            Vue.set(
              currObj.info,
              "supplier_id",
              response.data.return_purchase.supplier_id
            ),
            Vue.set(
              currObj.info,
              "supplier_name",
  ...
  watch: {
    ...
    "info.return_purchase_date": function (val, oldVal) {
      // console.log('estimate_date changes');
      if (this.errors != "") {
        this.errors["info.return_purchase_date"][0] = "";
      }
    },
...
.product-search-suggestion-return_purchase {
  position: absolute;
  /* background: #f4f3ef; */
  width: 100%;
  color: #212120;
...
.product-search-suggestion-return_purchase ul {
  list-style: none;
  margin: 0px;
  padding: 0px;
}

.product-search-suggestion-return_purchase ul li {
  padding: 10px;
  cursor: pointer;
  background: #f4f3ef;
}

.product-search-suggestion-return_purchase ul li:hover {
  background: #51cbce;
  color: white;
}

.product-search-suggestion-return_purchase::-webkit-scrollbar-track {
  -webkit-box-shadow: inset 0 0 6px rgba(0, 0, 0, 0.3);
  background-color: #f5f5f5;
}

.product-search-suggestion-return_purchase::-webkit-scrollbar {
  width: 6px;
  background-color: #f5f5f5;
}

.product-search-suggestion-return_purchase::-webkit-scrollbar-thumb {
  background-color: #000000;
}
</style>
