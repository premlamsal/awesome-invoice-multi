  ...
  <div>
    ...
    <p class="mb-4">
      ...
      <div class="card shadow mb-4">
        ...
        <div class="card-body" v-if="stockDetails.length > 0">
          <div class="table">
            <table class="table table-striped table-bordered" width="100%" cellspacing="0">
              ...
              <tbody>
                <tr v-for="stockDetail in stockDetails" v-bind:key="stockDetail.id">
                  <td>{{stockDetail.updated_at}}</td>
                  <td>{{stockDetail.product.name}}</td>
                  <td>{{stockDetail.action}}</td>
                  <td v-if="stockDetail.action==='Purchase'" style="background: #4CAF50;color:white">PUR-{{stockDetail.action_id}}</td>
                  <td v-else style="background: #F44336;color:white">INV-{{stockDetail.action_id}}</td>
                  <td>{{stockDetail.purchase_quantity}}</td>
                  <td>{{stockDetail.sale_quantity}}</td>
                  <td>{{stockDetail.price}}</td>
                  <td>{{stockDetail.in_stock}}</td>
                  <td>{{stockDetail.unit.short_name}}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  computed: {
    ...
    //  // this.stockDetails.grandTotal= this.stockDetails.reduce(function(carry, product) {
    //  // return carry + (parseFloat(product.quantity) * parseFloat(product.price));
    //  //       }, 0);
    //  // return this.stockDetails.grandTotal;
